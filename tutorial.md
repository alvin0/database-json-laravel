...
#### Code:
```php
use DatabaseJson\Classes\Database as DatabaseJson; /* I will skip this line in rest of code samples */
...
DatabaseJson::create('news', array(
    'topic' => 'string',
    'content' => 'string',
    'author_id' => 'integer'
));

DatabaseJson::create('comments', array(
    'content' => 'string',
    'author_id' => 'integer',
    'news_id' => 'integer'
));

DatabaseJson::create('tags', array(
    'name' => 'string'
));
```
...
```php
DatabaseJson::table('news')->findAll()->toArray();
```
#### Paginate
```php
DatabaseJson::table('news')->Pagination(5);
```
#### Paginate to array
```php
DatabaseJson::table('news')->Pagination(5)->toArrayWithPaginate();
```
...
RelationDatabaseJson::table('news')->belongsTo('users')->localKey('author_id')->foreignKey('id')->setRelation();
RelationDatabaseJson::table('news')->hasMany('comments')->localKey('id')->foreignKey('news_id')->setRelation();
RelationDatabaseJson::table('news')->hasAndBelongsToMany('tags')->localKey('id')->foreignKey('id')->setRelation();

/* relations for Users table */
RelationDatabaseJson::table('users')->hasMany('news')->localKey('id')->foreignKey('author_id')->setRelation();
RelationDatabaseJson::table('users')->hasMany('comments')->localKey('id')->foreignKey('author_id')->setRelation();

/* relations for Comments table */
RelationDatabaseJson::table('comments')->belongsTo('news')->localKey('news_id')->foreignKey('id')->setRelation();
RelationDatabaseJson::table('comments')->belongsTo('users')->localKey('author_id')->foreignKey('id')->setRelation();

/* relations for Tags table */
RelationDatabaseJson::table('tags')->hasAndBelongsToMany('news')->localKey('id')->foreignKey('id')->setRelation();
```
...
```php
$commment = DatabaseJson::table('comments');
...
```php
$junction = RelationDatabaseJson::table('news')->with('tags')->getJunction();
$tag_join = DatabaseJson::table($junction);
...
#### Code:
```php
$news = DatabaseJson::table('news')->with('users')->with('tags')->with('comments')->with('comments:users')->findAll();
